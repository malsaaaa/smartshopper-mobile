...
async function main() {
  ...
  console.log('--- VESAWIT PRODUCTS ---');
  for (const doc of products) {
    const name = doc.fields.name ? doc.fields.name.stringValue : '';
    if (name.toUpperCase().includes('VESAWIT')) {
      const parts = doc.name.split('/');
      const docId = parts[parts.length - 1];
      console.log(`Doc ID: ${docId}, Fields:`, JSON.stringify(doc.fields, null, 2));
    }
  }
  ...
  for (const doc of prices) {
    const parts = doc.name.split('/');
    const docId = parts[parts.length - 1];
    console.log(`Price Doc ID: ${docId}, Fields:`, JSON.stringify(doc.fields, null, 2));
  }
}

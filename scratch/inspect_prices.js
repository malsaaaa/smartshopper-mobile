...
async function main() {
  ...
  console.log(`Total price documents: ${priceDocs.length}`);
  
  if (priceDocs.length > 0) {
    console.log('Sample price document fields:', JSON.stringify(priceDocs[0].fields, null, 2));
  }

  const productDocs = await fetchCollection(token, 'products');
  console.log(`Total product documents: ${productDocs.length}`);
  if (productDocs.length > 0) {
    console.log('Sample product document fields:', JSON.stringify(productDocs[0].fields, null, 2));
  }
}
